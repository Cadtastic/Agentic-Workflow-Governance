...
*Skeleton ends here. v0.3 represents a complete first-pass framework with uniform depth across all components and a worked example. Recommended review focus: §3.2 RED-trigger stacking logic, §4.1 phase boundary criteria, §6.2 base-schema field set, and the worked example's gate composition (does it feel right at RED for an L2 / Medium-stakes / External workflow?).*
